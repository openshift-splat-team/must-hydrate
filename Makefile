...
ifeq ($(NO_DOCKER), 1)
  ...
  DOCKER_CMD := $(ENGINE) run --env GO111MODULE=$(GO111MODULE) --env GOFLAGS=$(GOFLAGS) --rm -v "$(PWD)":/go/src/github.com/openshift-splat-team/must-hydrate:Z  -w /go/src/github.com/openshift-splat-team/must-hydrate $(BUILD_IMAGE)
...
.PHONY: build
build: must_hydrate ## Build binaries

.PHONY: must_hydrate
must_hydrate:
	$(DOCKER_CMD) ./hack/go-build.sh must_hydrate
